...
**"Is this paper safe to submit?"** — Before sending any MSc-produced manuscript to a venue, run the **`final-review`** skill. It is the pre-submission integrity gate that catches the Generative-AI defects MSc can still introduce — fabricated/mismatched references, citations whose claims don't match the source, captions for not-reported figures, fabricated statistics, missing AI-disclosure. The in-pipeline `reviewer_agent` judges scientific quality; `final-review` judges *grounding*. Run both.
